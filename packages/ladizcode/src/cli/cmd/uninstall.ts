...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall ladizcode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Ladizcode")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g ladizcode-ai",
      pnpm: "pnpm uninstall -g ladizcode-ai",
      bun: "bun remove -g ladizcode-ai",
      yarn: "yarn global remove ladizcode-ai",
      brew: "brew uninstall ladizcode",
      choco: "choco uninstall ladizcode",
      scoop: "scoop uninstall ladizcode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "ladizcode", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".ladizcode") || binDir.includes(".opencode")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Ladizcode!")
}
